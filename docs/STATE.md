...
| S7 | in_progress | v2.1-dev: fdd6510 (WB 403 fix: сессия с прогревом, v5→v4→старый коллектор, WB_DEAD, тест warmup) | 🔴 АДАПТАЦИЯ одобрена и внесена, но WB с этой сети 498 на главной (X-Wbaas-Token: get) + 403 на search, Ozon 403 — блок уровня CDN/IP, JS-челлендж, не снять через requests; record_fixtures и шаг 3 (update_prices 81) отложены до сети/проверки человеком; 🔴 шаг 4: DOSE_PER_DAY/UNITS_PER_PACK реальных значений НЕТ (collect26/notebooks отсутствуют в репо, UNITS_PER_PACK выводился из названий офферов v1.x динамически) — нужна таблица от владельца или явный допуск DOSE=NORM/30; pytest 29 passed, 1 deselected (после commit fdd6510) | 2026-09-13 |

## Текущее состояние
- Ветка цикла: v2.1-dev (стартовый пакет S6 внесён; WB-адаптация S7 внесена).
...
- S6 (стартовый пакет) выполнен; открытые пункты: record_fixtures 403 WB (проверка человеком), дубль nightly/collect workflows (решение на мердже).
- S7 (шаги 3-4) БЛОКИРОВАН: WB 498/403 + Ozon 403 с этой сети (CDN/IP-блок); таблица DOSE/UNITS для 81 отсутствует (нет collect26/notebooks). Ждёт решения владельца: сеть/браузер для фикстур + источник DOSE/UNITS (или допуск DOSE=NORM/30).
- Следующая: S7 continue — после снятия блокад: record_fixtures рабочим путём → update_prices 81 (null≤5, флаги, ни один price не затёрт) → шаг 4 → коммиты.
