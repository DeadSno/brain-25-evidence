...
- S7 (прогон цен + DOSE/UNITS) ЧАСТИЧНО ВЫПОЛНЕН: basket-CDN карточки по id работают (avialable wbbasket.ru), DOSE_PER_DAY реальные (parsed/flags), monthly per-offer units. НЕ ВЫПОЛНЕНО: цикл прогон цен 81 ( шаг 3) + record_fixtures — БЛОК: WB 498/403 + Ozon 403 с этой сети, владелец запускает с хот-спота. CI-эксперимент (workflow_dispatch) — БЛОК: нет gh/токена, владелец запускает: `gh workflow run nightly_prices.yml --ref v2.1-dev`.
- Следующая: владелец выполняет record_fixtures + update_prices 81 с хот-спота, пилит результат + CI, после чего S7 закрывается, S8.
