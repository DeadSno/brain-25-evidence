...
|---|---|---|---|---|
| S0 | done | ce6f981 (MASTER_RUNBOOK) | приложение 4: полные тела 4.1-4.10 [СТАРТ-ПАКЕТ / ПОСЛЕДУЮЩАЯ ПОСЛЕДОВАТЕЛЬНОСТЬ] не вставлены владельцем — только заглушки; 20/26 добавок без цен ожидают ночного бота; фронт-fixes (надёжные) — только после S5 approve | 2026-09-13 |
| S1 | not_started | — | — | — |
...
- Следующая фаза: S1 [NEW SESSION] — аудит A (приложение 3.1) → docs/audit/01-map-data.md.

## Строки запуска фаз (из КАРТЫ ФАЗ)
- S1: аудит A (приложение 3.1) → docs/audit/01-map-data.md
