...
## Текущее состояние
- Ветка цикла: v2.1-dev (адаптация WB + basket-CDN + dose-parser + circuit breaker + tolerant fixtures + UI-verify внесены).
...
- S6 (стартовый пакет) выполнен.
- S7 (прогон цен + DOSE/UNITS) выполнен. НЕ ВЫПОЛНЕНО: физпрогон цен 81 (record_fixtures + update_prices) — владелец запускает с хот-спота. CI-эксперимент — владелец: `gh workflow run nightly_prices.yml --ref v2.1-dev`.
- S8 выполнен: circuit breaker, record_fixtures tolerant, UI-verify (скриншоты 1280+375).
- Следующая: S9 — e2e ALIGN, пирамида, snapshot, CHANGELOG, мердж v2.1-dev→main, тег v2.1, черновик поста; ТОЧКА РЕШЕНИЯ: «тегирую?».
