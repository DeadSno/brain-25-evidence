...
docs/ = GitHub Pages (index.html дашборд, map.html атлас). data.json = 130 добавок.
...
P38. Параллельная незакоммиченная работа НЕ участвует в чанке:
     её убирают в именованный git stash (или wip-ветку) ДО старта
     чанка и размораживают после мерджа. Урок v2.8.0: полуготовый
     openModal чуть не поехал в мобильный чанк — чанки атомарны,
     заготовки живут в морозилке.

P40. Терминал владельца = Windows PowerShell 5.1: цепочки команд
     только построчно или через `;` — `&&` не поддерживается (урок v2.8.0).

P41. PubMed-запросы для конвейера — только через оператор [tiab]
     (поиск в заголовке/абстракте), иначе каталог засоряется
     статьями, где добавка лишь упомянута как компаратор. Урок v2.8.0
     (гиалуроновая кислота): из 6 коротких абстрактов 5 нерелевантны.
     Правка: q = f'({term})[tiab] AND ...' в fetch_meta.py.
