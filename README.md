...
130/130 добавки — полная ручная верификация по методологии PRISMA:
...
- **Q1.4**: adv-поля 130/130 (about, who_needs, onset, myths, food_sources, guidelines, how_to_choose)
- **Механизмы**: 130/130, среднее 3.02 на карточку
- **Hedges' g**: 72/130 (68 с CI), двухпроходный алгоритм без cross-matching
...
**Что мы показываем для каждой из 130 добавок:**
...
| Hedges' g | Сила эффекта в стандартных отклонениях | Из мета-анализов (72/130) |
...
## Анализ конфликтов интересов (COI)

Проанализировано **6310 полных текстов** из PubMed Central (87% покрытие PMCID).

| Метрика | Значение | % |
|---------|----------|---|
| Указали COI | 5490 | **87.0%** |
| — нет конфликта | 4349 | 68.9% |
| — **реальный конфликт** | **467** | **7.4%** |
| — неясно | 674 | 10.7% |
| Не указали COI | 820 | 13.0% |
| Указали funding | 3500 | 55.5% |
| Упомянули фарм-компанию | 282 | 4.5% |

**ТОП-10 funders** — государственные фонды: NSFC (Китай), NIH/NCI/NIDDK/NIA (США),
NCCAM, NIAAA, NICHD, NIGMS. **Ни одной фарм-компании в ТОП-10.**
...
| Активных испытаний | проверены все 130 добавок |
...
│   ├── map.html                   # карта механизмов (130 узла + оверлей + кросс-связи)
│   ├── atlas.html                 # атлас: 25 хабов × 130 добавок
...
│   ├── dosage_parsed.json         # распарсенные дозы (81 из 130, остальные — fallback)
...
│   ├── config.py                  # 130 добавок: запросы, нормы
